...
try{    let token= req.header("accesstoken")
...
console.log(userId)
    ...
    let ticketCreated= await ticketModels.create({userId:userId,ticket:result1})

    return res.status(201).send({status:true,message:ticketCreated})
}
catch(error){
